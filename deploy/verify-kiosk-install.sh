...
HOTSPOT_ENV_PATH="${HOTSPOT_ENV_PATH:-/etc/solar-display/tailscale-hotspot-trigger.env}"
HOTSPOT_TRIGGER_PATH="${HOTSPOT_TRIGGER_PATH:-/usr/local/sbin/tailscale-hotspot-trigger.sh}"
HOTSPOT_SERVICE_PATH="${HOTSPOT_SERVICE_PATH:-/etc/systemd/system/tailscale-hotspot-trigger.service}"
HOTSPOT_TIMER_PATH="${HOTSPOT_TIMER_PATH:-/etc/systemd/system/tailscale-hotspot-trigger.timer}"
...
verify_hotspot_policy() {
  [[ -f "${HOTSPOT_ENV_PATH}" ]] || return 2

  (
    unset HOTSPOT_CONNECTION_ID HOTSPOT_SCAN_SSID HOTSPOT_PRIORITY
    # This file is root-owned deployment state generated by configure-hotspot-priority.sh.
    source "${HOTSPOT_ENV_PATH}" || return 1
    [[ -n "${HOTSPOT_CONNECTION_ID:-}" ]] || return 1
    [[ -n "${HOTSPOT_SCAN_SSID:-}" ]] || return 1
    [[ "${HOTSPOT_PRIORITY:-}" =~ ^-?[0-9]+$ ]] || return 1
    command -v nmcli >/dev/null 2>&1 || return 1
    nmcli connection show "${HOTSPOT_CONNECTION_ID}" >/dev/null 2>&1 || return 1
    [[ "$(nmcli -g 802-11-wireless.ssid connection show "${HOTSPOT_CONNECTION_ID}")" == "${HOTSPOT_SCAN_SSID}" ]] || return 1
    [[ "$(nmcli -g connection.autoconnect connection show "${HOTSPOT_CONNECTION_ID}")" == "yes" ]] || return 1
    [[ "$(nmcli -g connection.autoconnect-priority connection show "${HOTSPOT_CONNECTION_ID}")" == "${HOTSPOT_PRIORITY}" ]] || return 1
    [[ -x "${HOTSPOT_TRIGGER_PATH}" ]] || return 1
    [[ -f "${HOTSPOT_SERVICE_PATH}" ]] || return 1
    [[ -f "${HOTSPOT_TIMER_PATH}" ]] || return 1
    [[ "$(systemctl is-enabled tailscale-hotspot-trigger.timer 2>/dev/null)" == "enabled" ]] || return 1
  )
}
...
set +e
verify_hotspot_policy
hotspot_policy_status=$?
set -e
if [[ "${hotspot_policy_status}" == "0" ]]; then
  echo "OK: preferred hotspot profile and delayed trigger are configured"
elif [[ "${hotspot_policy_status}" == "2" ]]; then
  echo "SKIP: preferred hotspot policy is not configured"
else
  echo "FAIL: preferred hotspot profile or delayed trigger configuration drifted" >&2
  failures=$((failures + 1))
fi
