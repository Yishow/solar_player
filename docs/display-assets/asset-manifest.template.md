...
| `slot` | yes | `heroMedia`, `gallery`, `kpiIcon`, `flowNodeIcon`, `circuitNodeIcon`, `loadIcon`, `ornament`, or `placeholder`. |
| `assetKey` | yes | Stable kebab-case name. |
| `role` | yes | `photo`, `icon`, `ornament`, or `placeholder`. |
| `format` | yes | `webp`, `svg`, or justified exception. |
| `targetSize` | yes | Pixel size or SVG viewBox target. |
| `sourceMode` | yes | `seed-default`, `managed-asset`, or `direct-src`. |
| `promptRecipe` | yes | Recipe id used to generate or prepare the asset. |
| `version` | yes | Start with `v001`. |
