...
# Agent 工作指引（2026-07-03 重整）

單一事實來源在 `docs/ops/`；本檔只放路由與每次都要遵守的硬規則。`CLAUDE.md` 是本檔的同源分眾版（給 Claude Code）：**共用內容改動必同步、同一個 commit**；允許保留的分眾差異清單在 `docs/ops/maintenance.md`，不要為了「一字不差」把分眾差異抹掉。原版備份：`~/.claude/backups/2026-07-03-solar-player-institution/`。
...
| 要派 subagent、選 model（Claude Code 特有機制） | `docs/ops/dispatch.md` |
| 判斷完成沒、該不該問使用者、方向錯了沒 | `docs/ops/judgment.md` |
| 撰寫派工 prompt（Claude Code 特有機制） | `docs/ops/delegation.md` |
...
非 Claude 工具（Codex 等）沒有 subagent 機制時，跳過 dispatch/delegation 兩檔，其餘照讀。
