...
test("factory load rows and routing use shared display-family surface and token roles", () => {
  ...
  assert.match(factoryCss, /\.factory-circuit-routing-svg\s*\{[\s\S]*pointer-events:\s*none;/);
  ...
  assert.match(factorySource, /className="factory-circuit-routing-svg"/);
  assert.match(factorySource, /routingPaths\.map\(\(route\)/);
  assert.match(factorySource, /data-route-key=\{route\.key\}/);
  assert.doesNotMatch(factorySource, /FactoryCircuitRoutingReference/);
  assert.doesNotMatch(factorySource, /factory-routing-pv-inverter-reference/);
