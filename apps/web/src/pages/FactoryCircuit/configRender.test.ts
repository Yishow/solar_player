...
test("factory circuit runtime reads resolved display config for copy, status, nodes, connectors, load rows, and KPI cards", () => {
  ...
  assert.match(factoryCircuitSource, /resolvedConfig\.connectorTreatments\.solarToInverter/);
  assert.match(factoryCircuitSource, /resolvedConfig\.connectorTreatments\.inverterToBoard/);
...
test("factory circuit display page seed config captures the current default layout and hero contract", () => {
  ...
  assert.equal(config.connectorTreatments.inverterToBoard.strokeColor, "#527d3b");
