...
export function DeviceStatus() {
  ...
  const handleDiagnostic = async (action: "export-summary" | "refresh-readiness", label: string) => {
    ...
    setActionFeedback({
      detail: "正在同步 display diagnostics，請稍候。",
      title: label,
      tone: "loading"
    });
    try {
      ...
      setActionFeedback({
        detail: `${result.summary.skipSummary.count} skipped · ${result.summary.readinessSummary.blockingCount} blocking · ${result.summary.assetHealthSummary.unhealthyCount} unhealthy`,
        title: `${label}完成`,
        tone: "ready"
      });
  ...
  const viewModel = useMemo(
    ...
    [actionFeedback, displayOpsSummary, isLoading, status]
  );
