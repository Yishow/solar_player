...
export function SecuritySettings() {
  ...
  useEffect(() => {
    void getManagementPasswordState()
      .then((state) => {
  ...
  const submit = async () => {
    ...
    if ("error" in request) {
      setError(request.error ?? "輸入資料不完整。");
      return;
    }
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : "安全設定更新失敗。");
    }
  };

  return (
    <PageScaffold path="/settings/security" description="管理密碼與存取保護">
      <OpsSurface family="operations" className="mx-auto max-w-3xl space-y-6 p-8">
        ...
        <OpsInfoBanner
          ...
          detail={error || "管理密碼只會由 server 驗證與保存。"}
        />
        ...
        {form.enabled ? (
          <label className="block">
            新密碼
            <input
              className="mt-2 min-h-12 w-full rounded-md border border-neutral-300 px-4"
              type="password"
        ...
        <button
          className="min-h-12 rounded-md bg-neutral-900 px-6 font-semibold text-white"
          type="button"
          onClick={() => void submit()}
        >
          儲存安全設定
