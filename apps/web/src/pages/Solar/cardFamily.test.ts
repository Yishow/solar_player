...
test("solar KPI cards use the shared display card family", () => {
  ...
  assert.match(solarSource, /align=\{item\.cardStyle\.valueRowAlign\}/);
  assert.match(solarSource, /unit=\{isConfiguring \? "" : metric\.unit\}/);
  assert.match(solarSource, /value=\{isConfiguring \? displayPageCardConfiguringLabel : metric\.value\}/);
});
