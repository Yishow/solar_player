...
- [x] 1.2 Deliver `Invalidate registry consumers after registry mutations` by confirming `apps/server/src/routes/display-page-registry.ts` emits the expected registry-relevant signals and `apps/web/src/hooks/useDisplayPageRegistry.ts` consumes them, and verify with targeted route-host and editor regression tests.
...
- [x] 2.2 Deliver `Recompute route and editor definitions from the latest registry snapshot` end to end, and verify using `pnpm --filter @solar-display/web test -- src/pages/shared/displayPageRouteHost.test.ts src/pages/DisplayPagesEditor/runtimePageDefinitions.test.tsx`.
