...
- [ ] 2.1 Deliver `Evaluate conditional playback for display pages at runtime` and reference `Evaluate conditional playback at runtime with explicit skip reasons` by applying readiness, schedule, and health checks before page selection, verified by shared playback resolver tests and targeted server tests.
