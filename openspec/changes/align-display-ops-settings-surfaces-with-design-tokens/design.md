...
display-ops 管理面現在有兩條主線：一條是 `/display-pages/editor` 內的 page/asset/shell authoring，另一條是 `/settings/playback`、`/settings/images`、`/settings/mqtt`、`/settings/circuits`、`/slideshow-preview` 這些營運與預覽頁。前者已經承接資產庫與殼層裝飾工作流，而 `/settings/assets`、`/shell-decorations/editor` 只剩相容 handoff 角色。後者若不一起對齊 design tokens 與 page role，整體會出現「editor 像一套產品，settings 又像另一套產品」的斷裂。
...
### Preserve compatibility handoffs without restoring hidden footer destinations

這個 change 可以改善 `/settings/images` 對 editor 的 handoff，也可以調整 display-ops family 的導覽與 surface language；但不能把 `/settings/assets` 或 `/shell-decorations/editor` 重新放回主要 footer destination 集合，否則會和目前 editor-integrated workflow 衝突。
...
- display-ops family 的主要導航焦點維持在 playback/images/mqtt/circuits/preview；asset 與 shell authoring 透過 editor handoff 進入，不重新長回第一層 footer tab。
...
- 若 `/settings/assets` 或 `/shell-decorations/editor` 因此重新出現在主要 footer destinations，視為與現行資訊架構衝突。
...
- `apps/web/src/components/shellFoundation.test.ts` 驗證 management footer 仍不把 assets/shell handoff routes 當成第一層 destinations。
