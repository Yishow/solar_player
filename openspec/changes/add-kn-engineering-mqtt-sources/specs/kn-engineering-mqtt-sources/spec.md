...
The Player SHALL register KN engineering sources using the existing eight engineering IDs and an explicit sourceKind=engineering. Upstream acquisition, per-meter grouping and internal formulas SHALL remain publisher-owned. A reviewed engineering result MAY be authoritative even when calculated upstream. Player SHALL NOT require meterId, DDE Item, OPC NodeId or a copied CL device list to create or activate this source kind. Missing payload contracts SHALL leave the mode unconfigured rather than implying a physical-meter requirement.
