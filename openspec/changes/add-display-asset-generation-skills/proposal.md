...
Display 五頁的素材若靠臨場 prompt 或個人記憶產生，品質會不穩：圖片色調漂移、icon 線寬不一、素材命名混亂、版本不可追、preview 驗收缺少標準。最新 `main` 已新增 Codex multi-agent 架構與 `frontend_guide` agent；本 change 進一步以正規 skill package 形式建立 `display-asset-generation`，並以 `display_asset_guide` agent 作為可調用入口。

這個 change 要建立 display asset generation skill：把素材生成拆成 manifest、prompt recipe、post-process/export、preview QA 與版本管理。目標是讓 `Overview`、`Solar`、`FactoryCircuit`、`Images`、`Sustainability` 的 hero photos、gallery photos、icons、flow/circuit nodes、ornaments 能穩定產出並接入 seed/runtime config。

## What Changes

- 新增正規 skill package `skills/display-asset-generation/SKILL.md`，作為素材生成能力的單一技能入口。
...
- `display-asset-generation-skills`: 定義 display 五頁素材生成正規 skill package、Codex agent、manifest、prompt recipes、post-process/export 與 preview QA contract。
