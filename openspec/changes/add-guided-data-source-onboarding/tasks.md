...
- [ ] 1.1 **Draft** — 建立可返回task state，MQTT委派M2三階段與direct-edit共用驗證，錯誤/重試不清欄位。（U2-R1）
- [ ] 1.2 **Semantics** — 接E1 measurement kind、meter role、unit、cadence與scope欄位，只有一筆顯示等待基準。（U2-R2）
- [ ] 1.3 **Numbers** — numeric input保留字串直到validate，空白與0倍數不自動變1。（U2-R2）
...
- [ ] 1.7 **Persistence** — 單筆source save加入revision conflict；舊全量PUT相容但不可覆蓋他scope資料。（U2-R5）
- [ ] 1.8 **Impact** — 刪除或身份變更列出draft/live/derived引用，impact unknown阻擋；managed identity由server拒絕。（U2-R5）
- [ ] 1.9 **Handoff** — 保存成功產出safe source identity handoff與waiting-data/baseline狀態，U4接後續目標選取。（U2-R6）
- [ ] 1.10 **Security tests** — 測試preview前後DB與MQTT publish count不變，未授權mutation拒絕，URL不含秘密。（U2-R3 U2-R4 U2-R5 U2-R6）
- [ ] 1.11 **Verification** — 執行sourceDraft、sourceMappingPreview、settings-mqtt與DataHub相關tests及pnpm verify；驗證熟手可不走wizard。（U2-R1 U2-R2 U2-R3 U2-R4 U2-R5 U2-R6）

## 2. V2 Site-Setup Integration

- [ ] 2.1 **V2 Integration** — 接入E6/U6的唯一廠區計量設定與免手冊任務契約，完成本新增需求的API/UI整合與驗收情境。（U2-R7）
