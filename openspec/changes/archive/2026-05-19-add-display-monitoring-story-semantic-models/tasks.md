...
- [x] 1.2 Deliver `Keep shared monitoring story model diagnosable` by preserving fallback and binding reasons in reusable story outputs, verified by targeted tests that inspect diagnostic fields.
...
- [x] 2.2 Deliver `Provide flow state storytelling for Solar` and `Support comparison targets in Solar story blocks` by extending `Solar` story mapping with flow-state and comparison-target semantics, verified by `pnpm --filter @solar-display/web test -- src/pages/Solar/viewModel.test.ts`.
...
- [x] 3.2 Deliver `Provide alert reasons for Factory Circuit load rows` by adding deterministic alert reasons and missing-data states for bound rows, verified by `pnpm --filter @solar-display/web test -- src/pages/FactoryCircuit/viewModel.test.ts` and targeted diagnostic assertions.
