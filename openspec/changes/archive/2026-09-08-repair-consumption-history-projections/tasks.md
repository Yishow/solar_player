...
- 測試驗證：
  1. `pnpm --filter @solar-display/shared test` (155 測通過，exit code 0)
     - `E2-R1 day 300 / month 4300 / year 8300 from cumulative samples`
     - `E2-R2 Taipei September start is 2026-08-31T16:00:00Z`
  2. `pnpm --filter @solar-display/server test` (925 測通過，exit code 0)
     - `metrics-history.test.ts` (API 邊界與 periodSummary 輸出)
     - `energyAuthoringJourney.test.ts` (shadow projection 啟用與回滾交易測試)
     - `persisted period freshness follows configured policy without rewriting observations`
