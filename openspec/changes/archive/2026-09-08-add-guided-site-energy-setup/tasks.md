...
- 測試驗證：
  1. `pnpm --filter @solar-display/shared test` (155 測通過，exit code 0)
     - `U6 four-step factory energy setup keeps scope in the entry href`
  2. `pnpm --filter @solar-display/web test` (1440 測通過，exit code 0)
     - `SiteEnergySetupPanel.test.tsx` (四步廠區用電設定流程、conflict & preview token 重試、meter picker 綁定 channelId)
