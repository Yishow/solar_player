...
狀態：已完成；admission gate、retained replay 隔離、M2 selector extractor 串接與全量驗證已全數通過。
...
- [x] 1.4 **Ingestion** — 在去重、counter continuity 與任何 accepted state mutation 之前加入 admission gate；帶可信 source timestamp 的 retained 十次只接受一次，無可信 source timestamp 的 retained 十次全部回 RETAINED_SOURCE_TIME_UNKNOWN。以 E1-R2-S03 重啟隔離 broker＋SQLite fixture 驗證 10000 舊 retained 不覆蓋 10100、不刷 freshness、不改 baseline/epoch、不送事件或製造 discontinuity；不以 snapshot poll 製造讀值。（E1-R2） 對照：Immutable samples and idempotent ingestion。
...
- 測試：`pnpm --filter @solar-display/server test src/services/meterReadingService.test.ts src/services/mqttMeterIngest.test.ts src/mqtt/metricKeyIngestion.test.ts src/services/energyAuthoringJourney.test.ts` — exit 0（37 pass）。
- 覆蓋重點：
  - Task 1.4：`E1-R2-S01 repeated retained payload with source time is accepted once`、`E1-R2-S03 restart retained replay without timestamp does not overwrite 10100`、`E1 production callback gates retained replay, collision and late samples before generic live writes`（10000 舊 retained 重送 10 次隔離為 quarantine，不覆蓋 10100、不刷 freshness、不送事件）。
  - Task 1.12：`E1-R7 production MQTT callback uses M2 extractor then E1 admission`、`M2 apply persists selector then tagged MAIN ingest updates only that meter`、`E1 preserves selected record timestamp path and selector version`、`E1 an explicit tagged selector cannot borrow a timestamp from another record or accept a raw scalar`、`Q1 CL+KN ingest→E2→E3→shares isolate 300/4300/8300 and 50/30/20`。
