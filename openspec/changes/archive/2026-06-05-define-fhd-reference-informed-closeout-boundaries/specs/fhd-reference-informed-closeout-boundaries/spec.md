...
A reference difference classified as `actual-gap` SHALL record the surface, gap type, implementation consequence, and verification gate. The gap type SHALL identify whether the blocker is editor capability, runtime parity, asset or content state, fallback behavior, publish refresh, visual tuning, or handoff evidence.
