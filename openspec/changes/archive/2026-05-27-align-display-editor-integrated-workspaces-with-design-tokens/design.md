...
這一輪整合已經把 `/settings/assets` 與 `/shell-decorations/editor` 的主要工作流帶回 `/display-pages/editor`，而且 media effects 也已經有 canonical `屬性` / `來源連接` surface。現在的問題不是入口還沒整合，而是 UI 還停在過渡態：editor canvas card、asset workspace、shell workspace、media effect surfaces、source connection panel 之間缺乏一致的 token 語言，也缺少「我現在在哪個工作區、手上的 selection 是誰、下一步能做什麼」這種高密度操作線索。
...
- 不把 `/settings/assets` 或 `/shell-decorations/editor` 重新變回主要 footer destinations。
...
editor 自身、asset workspace、shell workspace、source connection / properties rail、media effect inspector / summary surfaces，應共享一套 surface primitives，例如 context board、selection detail board、sticky action bar、empty state、blocked state、workspace section heading。這能避免每個 workspace 長成不同語言的拼裝頁。

### Keep compatibility entry routes as handoffs, not restored standalone navigation

`/settings/assets` 與 `/shell-decorations/editor` 已經是導回 `/display-pages/editor` workspace 的相容入口。這個 change 可以調整它們進入 editor 後的 surface 對齊與回程體驗，但不能把它們重新做成第一層 management footer 導覽。
...
- 右側 `屬性` / `來源連接` surfaces 在對齊 tokens 後，仍保留單一責任：`屬性` 負責編輯，`來源連接` 負責摘要與跳轉。
- `/settings/assets` 與 `/shell-decorations/editor` 仍作為 handoff route 進入整合 workspace，而不是重新出現在主要 footer destinations。
...
- 若相容入口被重新拉回 footer 當成主要 destinations，視為與現行整合方向衝突。
...
- `apps/web/src/components/shellFoundation.test.ts` 驗證相容入口不會重新出現在 management footer。
